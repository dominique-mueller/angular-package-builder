 ...
 */
export function expectInlineTemplate( distPath: string, packageName: string, sourcePath: string, componentType: string, expectedStyle: string ): void {
	...
	it( 'should inline into ES2015 build', async() => {
		...
		expect( file.getInlineTemplates() ).toContain( expectedStyle );
	} );
	...
	it( 'should inline into Angular Metadata', async() => {
		...
		expect( file.getInlinedTemplate( componentType ) ).toContain( expectedStyle );
	} );
