...
/**
 * Flat ESM2015 Module - Unit Test
 */
describe( 'Flat ESM2015 Module (with Source Maps)', () => {

	const distSubfolder: string = 'fesm2015';
	...
	beforeAll( async () => {
		moduleFileContent = await import( path.join( importPath, distSubfolder, 'my-library.js' ) );
		sourceMapFile = await fs.readFileSync( path.join( distFolderPath, distSubfolder, 'my-library.js.map' ), 'utf-8' );
