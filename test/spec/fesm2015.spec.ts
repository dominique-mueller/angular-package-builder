...
describe( 'Flat ESM2015 Module (with Source Maps)', () => {
	...
	beforeAll( async () => {
		moduleFileContent = await import( path.join( importPath, 'fesm2015', 'my-library.js' ) );
		sourceMapFile = await fs.readFileSync( path.join( distFolderPath, 'fesm2015', 'my-library.js.map' ), 'utf-8' );
