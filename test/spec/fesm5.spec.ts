...
describe( 'Flat ESM5 Module (with Source Maps)', () => {
	...
	beforeAll( async () => {
		moduleFileContent = await import( path.join( importPath, 'fesm5', 'my-library.js' ) );
		sourceMapFile = await fs.readFileSync( path.join( distFolderPath, 'fesm5', 'my-library.js.map' ), 'utf-8' );
